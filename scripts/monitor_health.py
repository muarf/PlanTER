    ...
    if is_error:
        ...
        if os.path.exists(STATE_FILE):
            try:
                ...
                pass
            send_alert("✅ [TER Finder] L'API est de nouveau opérationnelle et saine.")
...
def check_health():
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "TER-Finder-Monitor"})
        with urllib.request.urlopen(req, timeout=10) as response:
            if response.status != 200:
                notify(f"❌ [TER Finder] L'API renvoie un code HTTP {response.status} sur {url} !")
                return
            content = response.read().decode("utf-8")
    except Exception as e:
        notify(f"❌ [TER Finder] L'API est injoignable sur {url} !\nErreur : {e}")
        return

    try:
        data = json.loads(content)
    except Exception as e:
        notify(f"❌ [TER Finder] Impossible de parser la réponse JSON de {url} !\nErreur : {e}")
        return

    # Check status
    if data.get("status") != "ok":
        notify(f"⚠️ [TER Finder] Statut anormal de l'API : {data.get('status')}")
        return
    ...
    if last_refresh:
        ...
        if status in ("error", "degraded"):
            notify(f"⚠️ [TER Finder] Le dernier refresh hebdomadaire a échoué (statut: {status}).")
            return
    ...
    if realtime:
        if not realtime.get("polling"):
            notify("⚠️ [TER Finder] Le polling temps réel GTFS-RT n'est pas actif !")
            return
        elif not realtime.get("fresh"):
            notify(f"⚠️ [TER Finder] Le flux temps réel GTFS-RT n'est plus frais (âge: {realtime.get('age_s')}s) !")
            return

        alerts = realtime.get("alerts")
        if alerts and not alerts.get("fresh") and alerts.get("count", 0) > 0:
            notify(f"⚠️ [TER Finder] Le flux d'alertes de service n'est plus frais (âge: {alerts.get('age_s')}s) !")
            return
    ...
    if coverage_end:
        try:
            ...
            if days_left <= 7:
                notify(f"⚠️ [TER Finder] Les données GTFS expirent bientôt ! Fin de couverture : {coverage_end} (dans {days_left} jours).")
                return
        except Exception:
            pass
